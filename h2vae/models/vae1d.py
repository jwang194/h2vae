"""1D convolutional VAE for time-series or sequence data."""
...
class Conv1dCellDown(nn.Module):
    """Two-conv downsampling cell: Conv1d(stride=1) -> Conv1d(stride=2)."""

    def __init__(self, ni: int, no: int, ks: int = 3, act: str = "elu"):
        super().__init__()
        self.conv1 = nn.Conv1d(ni, no, kernel_size=ks, stride=1, padding=1)
        self.conv2 = nn.Conv1d(no, no, kernel_size=ks, stride=2, padding=1)
...
class Conv1dCellUp(nn.Module):
    """Two-conv upsampling cell: interpolate(2x) -> Conv1d -> Conv1d."""

    def __init__(self, ni: int, no: int, ks: int = 3,
                 act1: str = "elu", act2: str = "elu"):
        super().__init__()
        self.conv1 = nn.Conv1d(ni, no, kernel_size=ks, stride=1, padding=1)
        self.conv2 = nn.Conv1d(no, no, kernel_size=ks, stride=1, padding=1)
...
@register("vae1d")
class VAE1D(BaseVAE):
    """1D convolutional VAE for sequence / time-series data.

    Args:
        img_size: Sequence length. Must be divisible by ``2**steps``.
            Named ``img_size`` for consistency with the ``vae_cfg`` dict
        ...
        colors: Number of input channels (streams).
    ...
    def __init__(
        ...
        self.red_seq_len = img_size // (2 ** steps)
        self.nf = nf
        ...
        self.econv.append(Conv1dCellDown(colors, nf, ks, act))
        for _ in range(steps - 1):
            self.econv.append(Conv1dCellDown(nf, nf, ks, act))

        # Decoder
        self.dconv = nn.ModuleList()
        for _ in range(steps - 1):
            self.dconv.append(Conv1dCellUp(nf, nf, ks, act1=act, act2=act))
        self.dconv.append(Conv1dCellUp(nf, colors, ks, act1=act, act2="linear"))
    ...
    def encode(self, x: Tensor) -> tuple[Tensor, Tensor]:
        """Encode sequences to latent mean and std.

        Args:
            x: Sequences of shape ``(batch, colors, seq_length)``.
    ...
    def decode(self, z: Tensor, external: Tensor | None = None) -> Tensor:
        ...
        Returns:
            Reconstructed sequences ``(batch, colors, seq_length)``.
        """
        if external is not None:
            z = torch.hstack((z, external))
        x = self.dense_dec(z)
        x = x.view(-1, self.nf, self.red_seq_len)
